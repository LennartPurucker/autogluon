...
class AbstractTrainer:
    ...
    def get_model_learning_curves(self, model: str, child_model: str | None = None) -> dict:
        model_type = self.get_model_attribute(model=model, attribute="type")
        model_path = self.get_model_attribute(model=model, attribute="path")
        path = os.path.join(self.path, model_path)
        if child_model is not None:
            path = os.path.join(path, child_model)
        return model_type.load_learning_curves(path=path)
        ...
        try:
            ...
            if not compute_score:
                ...
                if model.y_pred_proba_val_oof_ is not None:
                    # TODO: re-implement in the same way below
