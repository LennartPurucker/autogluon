...
def clean_oof_predictions(X: pd.DataFrame, y: pd.Series, feature_metadata: FeatureMetadata, problem_type: str) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    if problem_type == BINARY:
        # TODO: verify assumption that proba are always positive class proba
        for f in stack_cols:
        ...
        if class_overflow:  # needs to hold
            raise ValueError(f"The OOF predictions contain classes that are not in the label! The follow missing classes are missing in the label: {class_overflow}")
