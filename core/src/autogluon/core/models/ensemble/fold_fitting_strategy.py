...
class FoldFittingStrategy(AbstractFoldFittingStrategy):
    ...
    def _predict_oof(self, fold_model, fold_ctx, is_val_predict=False):
        ...
        if self.time_limit is not None:
            if not is_last_fold:
                ...
                if expected_remaining_time_required > time_left:
                    raise TimeLimitExceeded
        pred_proba = fold_model.predict_proba(X_val_fold, is_val_predict=is_val_predict)
...
class SequentialLocalFoldFittingStrategy(FoldFittingStrategy):
    ...
    def _fit_fold_model(self, fold_ctx):
        ...
        fold_model, pred_proba = self._predict_oof(fold_model, fold_ctx, is_val_predict=True)
...
def check_and_clean_oof(X_fold, y_fold, X_val_fold, y_val_fold, fold_model, kwargs_fold, smooth_fitter=False):
    ...
    if kwargs_fold["clean_oof_predictions"] and stack_cols:
        ...
        ir_map = clean_oof_predictions(
            ...
            fold_model=None if smooth_fitter else fold_model ,
...
def _ray_fit(
    *,
    ...
    fold_model, pred_proba = _ray_predict_oof(fold_model, X_val_fold, y_val_fold, time_train_end_fold, resources["num_cpus"], save_bag_folds, is_val_predict=True)
...
def _ray_predict_oof(fold_model, X_val_fold, y_val_fold, time_train_end_fold, num_cpus=-1, save_bag_folds=True, is_val_predict=False):
    pred_proba = fold_model.predict_proba(X_val_fold, num_cpus=num_cpus, is_val_predict=is_val_predict)
...
class ParallelFoldFittingStrategy(FoldFittingStrategy):
    ...
    def __init__(self, *, num_jobs: int, num_folds_parallel: int, max_memory_usage_ratio: float = 0.8, model_sync_path: Optional[str] = None, **kwargs):
        ...
        self._psuedo_sequential : bool = num_folds_parallel == 1
    ...
    def _update_bagged_ensemble_times(self):
        ...
        self.bagged_ensemble_model._add_parallel_child_times(fit_time=self.fit_time, predict_time=self.predict_time,
                                                             predict_1_time=self.predict_1_time)
    ...
    def _run_psuedo_sequential(self, X, y, X_pseudo, y_pseudo, model_base_ref, time_limit_fold, head_node_id):
        """
        ...
        overhead. Here, at most, we have the overhead of 1 fold. In the case of `_run_parallel` the asynchronous
    ...
    def after_all_folds_scheduled(self):
        ...
        if self._psuedo_sequential:
            logger.log(
                30,
                f"\tSwitch to psuedo sqeuntial ParallelFoldFittingStrategy to avoid Python memory leakage.\n"
                f"\tOverule this behavior by setting fold_fitting_strategy to 'sequential_local' in ag_args_ensemble when when calling predictor.fit",
            )
            self._run_psuedo_sequential(X, y, X_pseudo, y_pseudo, model_base_ref, time_limit_fold, head_node_id)
        else:
