...
class SlidingBoundariesArchive(ArchiveBase):
    ...
    def __init__(self, dims, ranges, seed=None, dtype=np.float64, remap_frequency=100, initial_remap=100, buffer_capacity=1000, show_analysis=False):
        self._dims = np.array(dims)
        if len(self._dims) != len(ranges):
            raise ValueError(f"dims (length {len(self._dims)}) and ranges " f"(length {len(ranges)}) must be the same length")
        ...
        self._default_remap_frequency = remap_frequency
        self._remap_frequency = initial_remap
        self._first_remap_done = False

        # Allocate an extra entry in each row so we can put the upper bound at
        # the end.
        self._boundaries = np.full((self._behavior_dim, np.max(self._dims) + 1), np.nan, dtype=self.dtype)

        # Initialize the boundaries.
        for i, (dim, lower_bound, upper_bound) in enumerate(zip(self._dims, self._lower_bounds, self._upper_bounds)):
    ...
    def _get_index_numba(behavior_values, upper_bounds, lower_bounds, boundaries, dims):
        ...
        """
        behavior_values = np.minimum(np.maximum(behavior_values + _EPSILON, lower_bounds), upper_bounds - _EPSILON)
        index = []
    ...
    def get_index(self, behavior_values):
        ...
        """
        index = SlidingBoundariesArchive._get_index_numba(behavior_values, self.upper_bounds, self.lower_bounds, self._boundaries, self._dims)
        return tuple(index)
    ...
    def _remap(self):
        ...
        SlidingBoundariesArchive._remap_numba_helper(sorted_bc, self._buffer.size, self._boundaries, self._behavior_dim, self.dims)
    ...
    def add(self, solution, objective_value, behavior_values, metadata=None):
        ...
        if self._total_num_sol % self._remap_frequency == 0:
            if self._show_analysis and (not self._first_remap_done):
