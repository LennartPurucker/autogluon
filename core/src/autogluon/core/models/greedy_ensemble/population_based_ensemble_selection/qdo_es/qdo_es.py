...
class QDOEnsembleSelection(AbstractWeightedEnsemble):
    ...
    base_models_metadata:
        Meta data for diversity measures (advanced usage)
    ...
    def __init__(
        ...
        archive_type: str = "quality",
        buffer_ratio: float = 1.0,
        max_elites: int = 49,
        emitter_vars: Optional[dict] = None,
        batch_size: int = 40,
        ...
        **kwargs,
        ...
        if self.behavior_space.requires_base_model_metadata:
            ...
            if self.base_models_metadata is None:
                raise ValueError("Base model metadata is required for this behavior space but is None right now.")
            for bm_metadata in self.base_models_metadata:
                if not (("auto-sklearn-model" in bm_metadata) and bm_metadata["auto-sklearn-model"]):
                    raise NotImplementedError("We currently only support base model metadata created by the ", "Auto-sklearn Assembler.")
                self.qdo_base_models_metadata_.append(bm_metadata["config"])

            # -- Compute ranges
            all_numeric_keys = [(k, v) for bm in self.base_models_metadata for k, v in bm["config"].items() if not isinstance(v, str)]
            # Get bounds
        ...
        if (n_jobs == 1) or (os.name == "nt"):
            ...
            if n_jobs == -1:
                n_jobs = len(os.sched_getaffinity(0))  # crashes on mac
            self._n_jobs = n_jobs
            self._use_mp = True

    def fit(self, predictions: List[np.ndarray], labels: np.ndarray, time_limit=None, sample_weight=None) -> AbstractWeightedEnsemble:

        # Hacky way to adjust input to be as expected by the emitter (as we removed the original wrapper class...)
        # -- Make sure it is 2D for binary classification
        if self.problem_type == 'binary':
            # FIXME: verify assumption that AG will always have the greater label as positive class internally.
            new_pred = []
        ...
        self.internal_n_iterations, self.n_rest_evaluations = self._compute_n_iterations(self.n_evaluations, self.batch_size)
        ...
        if (self.archive_type != "quality") and self.behavior_space.empty:
            raise ValueError("The behavior space must not be empty if the archive type is not quality!")
        ...
        if self.emitter_method not in self.allowed_emitter_methods:
            raise ValueError("The emitter methods is not in {}. Got: {}".format(self.allowed_emitter_methods, self.emitter_method))

        if self.emitter_initialization_method not in self.allowed_emitter_initialization_methods:
            raise ValueError(
                "The emitter initialization method is not in {}. Got: {}".format(
                    self.allowed_emitter_initialization_methods, self.emitter_initialization_method
                )
            )

        if (not isinstance(self.buffer_ratio, float)) or (isinstance(self.buffer_ratio, float) and ((self.buffer_ratio <= 0) or (self.buffer_ratio > 1))):
            raise ValueError("Buffer ratio must be a float larger zero and smaller or equal to 1.", f"Got: {self.buffer_ratio}, {type(self.buffer_ratio)}")
        ...
        if (self.archive_type not in ["quality"]) and int(
            ...
            raise ValueError("Number of max elites can not be used to split the dimensions evenly!", f"Got: {self.max_elites}, {self.behavior_space.n_dims}.")
    ...
    def _init_archive(self):
        ...
        if self.archive_type == "sliding":
            archive = SlidingBoundariesArchive(
                dims=[int(round(self.max_elites ** (1.0 / self.behavior_space.n_dims))) for _ in range(self.behavior_space.n_dims)],
    ...
    def _update_n_iterations(self, n_evals_outside_of_loop: int):
        ...
        self.internal_n_iterations, self.n_rest_evaluations = self._compute_n_iterations(self.n_evaluations - self._n_init_evals, self.batch_size)
    ...
    def _init_emitters(self, predictions, y_true):
        ...
        # Code for analysis
        if self.show_analysis:
            o, b = self._evaluate_batch_of_solutions(np.array([w for w, _ in _start_weight_vectors]), predictions, y_true)
        ...
        if self.emitter_method == "DiscreteWeightSpaceEmitter":
            ...
            for req_time_atr in ["mutation_probability_after_crossover", "crossover_probability"]:
                if (req_time_atr in self.emitter_vars) and (isinstance(self.emitter_vars[req_time_atr], Callable)):
                    self.emitter_vars[req_time_atr] = partial(self.emitter_vars[req_time_atr], max_time=self.internal_n_iterations)
        ...
        metrics_need_proba = self.score_metric.needs_proba or ("proba" in self.behavior_space.required_prediction_types)
    ...
    def _init_optimize(self, predictions):
        ...
        if "raw" in self.behavior_space.required_prediction_types:
            self._raw_predictions = [custom_confidences_to_predictions(bm) for bm in predictions]

    def _optimize(self, predictions: List[np.ndarray], labels: np.ndarray, time_limit) -> None:
    ...
    def _evaluate_batch_of_solutions(self, solutions: np.ndarray, predictions, y_true):
        ...
        func_args = [
            predictions,
            y_true,
            partial(self._calculate_regret, metric=self.score_metric, sample_weight=self.sample_weight, return_score=True),
            self.behavior_space,
...
def _pool_init(
    ...
    global predictions, y_true, score_metric, behavior_space, normalize_predict_proba_, proba_req, raw_req, raw_predictions, bm_meta_req, config_key_to_range_, qdo_base_models_metadata_, solutions
...
def evaluate_single_solution(
    ...
    # Get Score
    y_pred_ensemble = _clean_ensemblepredict(predictions, weight_vector, normalize_predict_proba_)

    # Negative loss because we want to maximize
    s_m = score_metric(y_true, y_pred_ensemble)
    ...
    if raw_req:
        pred_arguments["raw_preds"] = [
            custom_confidences_to_predictions(y_pred_ensemble),
    ...
    if bm_meta_req:
        pred_arguments["input_metadata"] = [config_key_to_range_, [md for md, selected in zip(qdo_base_models_metadata_, rm_zero_mask) if selected]]
...
def _clean_ensemblepredict(predictions: List[np.ndarray], weights: np.ndarray, normalize_predict_proba: bool = False) -> np.ndarray:
...
def custom_confidences_to_predictions(confidences):
