    ...
    def __call__(
        ...
        if isinstance(self, OrdinalClassificationDiversityMetric):
            ...
            if checks:
                ...
                if (y_pred_ensemble is not None) and any([bm.shape[1] < y_pred_ensemble.shape[1] for bm in Y_pred_base_models]):
                    raise ValueError("y_true has more classes than some base model has predicted for.")

                if (y_pred_ensemble is not None) and any([bm.shape[1] != y_pred_ensemble.shape[1] for bm in Y_pred_base_models]):
                    raise ValueError("The number of classes does not match between ensemble and some base model predictions.")
            ...
            diversity_score = self.metric_func(y_true, Y_pred_base_models, **kwargs)
        elif isinstance(self, RegressionDiversityMetric):
            if checks:
                Y_pred_base_models = [_check_y(Y_pred_base_model) for Y_pred_base_model in Y_pred_base_models]
...
class OrdinalClassificationDiversityMetric(DiversityMetric):
    def __init__(self, metric_func, metric_name, requires_weights, requires_y_ensemble_pred, more_diversity_if_higher, single_model_ensemble_default):
        super().__init__(
            metric_func,
            metric_name,
...
class RegressionDiversityMetric(DiversityMetric):
    def __init__(self, metric_func, metric_name, requires_weights, requires_y_ensemble_pred, more_diversity_if_higher, single_model_ensemble_default):
        super().__init__(
            metric_func,
            metric_name,
...
def make_diversity_metric(
    ...
    if metric_type == "OrdinalClassification":
        d_m = OrdinalClassificationDiversityMetric(
            metric_func, metric_name, requires_weights, requires_y_ensemble_pred, more_diversity_if_higher, single_model_ensemble_default
        )
...
# - Error Correlation
def average_loss_correlation(y_true, Y_pred_base_models: List[np.ndarray], aggregation_method=np.mean):
    ...
    if len(lb.classes_) != Y_pred_base_models[0].shape[-1]:
        print("Difference in ensemble and base model classes found.", "Trying to fix it by setting ensemble classes to base model classes.")
...
def average_loss_correlation_regression(y_true, Y_pred_base_models: List[np.ndarray], aggregation_method=np.mean):
    # The above but for regression
    loss_per_bm = [abs(y_true - bm_pred) for bm_pred in Y_pred_base_models]
    corr_matrix = np.abs(np.corrcoef(loss_per_bm))
    upper = corr_matrix[np.triu(np.ones(corr_matrix.shape), k=1).astype(np.bool)]
    upper = np.nan_to_num(upper)
    return aggregation_method(upper)
...
# --- Initialization
LossCorrelation = make_diversity_metric("OrdinalClassification", "Average Loss Correlation", average_loss_correlation)
LossCorrelationRegression = make_diversity_metric("Regression", "Average Loss Correlation", average_loss_correlation_regression)
