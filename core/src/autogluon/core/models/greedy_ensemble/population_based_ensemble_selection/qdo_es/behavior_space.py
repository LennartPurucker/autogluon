...
class BehaviorFunction:
    ...
    def __init__(
        self, function: Callable[[], float], required_arguments: List[str], range_tuple: Tuple[float, float], required_prediction_format: str, name: str = None
    ):
        self.function = function

        if any(v not in ALLOWED_ARGUMENTS for v in required_arguments):
            raise ValueError("Not allowed argument name used. Allowed are: {}. Got: {}".format(ALLOWED_ARGUMENTS, required_arguments))
        ...
        if required_prediction_format not in ["raw", "proba", "none"]:
            raise ValueError("Unknown prediction format. Expected: {}. Got: .{}".format(["raw", "proba", "none"], required_prediction_format))
