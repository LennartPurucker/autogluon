...
class DiscreteWeightSpaceEmitter(EmitterBase):
    ...
    elite_selection_method: str in {"deterministic", "combined_dynamic", "tournament"}, default="deterministic"
    ...
    crossover: Optional[str] in {"average", "two_point_crossover"}, default=None
    ...
    crossover_probability: Union[float, Callable], default=1.
    ...
    crossover_probability_dynamic: bool, default=False
    ...
    mutation_probability_after_crossover: Union[float, Callable], default=1.
        ...
        Internally, all values are converted to a function that returns the mutation probability.
    mutation_probability_after_crossover_dynamic: bool, default=False
    ...
    def __init__(
        ...
        elite_selection_method: str = "deterministic",
        crossover: Optional[str] = None,
        crossover_probability: Union[float, Callable] = 1.0,
        crossover_probability_dynamic: bool = False,
        mutation_probability_after_crossover: Union[float, Callable] = 1.0,
        mutation_probability_after_crossover_dynamic: bool = False,
        negative_steps: bool = False,
        shared_seen_resources: Optional[Tuple[set, dict]] = None,
        ...
        if elite_selection_method not in self.allowed_elite_selection_methods:
            raise ValueError("Unknown elite_selection_method! Allowed are: {}. Got: {}.".format(self.allowed_elite_selection_methods, elite_selection_method))
        ...
        if (not isinstance(mutation_probability_after_crossover, float)) and (not isinstance(mutation_probability_after_crossover, Callable)):
            raise ValueError("mutation_probability_after_crossover must be float or  Callable", f"Got: {type(mutation_probability_after_crossover)}")
        ...
        if (not isinstance(crossover_probability, float)) and (not isinstance(crossover_probability, Callable)):
            raise ValueError("crossover_probability must be float or  Callable", f"Got: {type(crossover_probability)}")
        if isinstance(crossover_probability, float) and ((crossover_probability <= 0) or (crossover_probability > 1)):
            raise ValueError("for crossover_probability it must hold that hold that 0 < crossover_probability <=1", f"Got: {crossover_probability}")
        ...
        if crossover_probability_dynamic and (not isinstance(crossover_probability, float)):
            raise ValueError("crossover_probability_dynamic can only be used if crossover_probability is a float!")
        self._crossover_probability_dynamic = crossover_probability_dynamic

        if mutation_probability_after_crossover_dynamic and (not isinstance(mutation_probability_after_crossover, float)):
            raise ValueError("_mutation_probability_after_crossover_dynamic can only be " + "used if mutation_probability_after_crossover is a float!")
        self._mutation_probability_after_crossover_dynamic = mutation_probability_after_crossover_dynamic
        ...
        if shared_seen_resources is not None:
            ...
            self._seen_percentages = set(hash(tuple(w)) for w, _ in self._start_weight_vectors)
        ...
        self._weight_random_step_selection = weight_random_step_selection  # weight_random_step_selection
        if self._weight_random_step_selection:
            np.seterr(invalid="ignore")  # since we abuse dividing by zero to obtain nan values.
            self._base_model_sample_weights = np.full(n_base_models, 1 / n_base_models)  # step direction weights
        ...
        self._origin_performance_template = {
            SolutionOrigins.no_crossover: [0, 0, 0],  # Objective value, improvement count, total count
    ...
    def mutation_probability_after_crossover(self):
        try:
            ...
            pass
        return self._mutation_probability_after_crossover
    ...
    def ask(self):
        ...
        self._emergency_step_size = 0
        self._emergency_mutation_rate = 0  # Only use when cross is not None and mutation_probability_after_crossover is 0
        ...
        while len(w_vecs_to_return) < self._batch_size:
            ...
            if self._start_weight_vectors:
                ...
                for (proposed_float_weights, new_sample_size, reject_flag), sol_org, e_org in mutated_children:
                    # Handle reject
                    if reject_flag:
        ...
        self._elite_origin_reject_counter = {
            k: self._tmp_elite_origin_reject_counter[k] + self._elite_origin_reject_counter[k] for k in self._elite_origin_reject_counter.keys()
        }

        # -- Important return values to keep state inbetween ask/tell
        self.last_ask_sample_sizes = number_of_steps  # Required to correctly transform continuous to discrete weights
        ...
        self._mutation_probabilities_after_crossover_over_time.append(self.mutation_probability_after_crossover)
        self._crossover_probabilities_over_time.append(self.crossover_probability)
        self._random_elite_selection_probabilities_over_time.append(self._random_elite_selection_probability)
    ...
    def tell(self, solutions, objective_values, behavior_values, metadata=None):
        ...
        if (metadata is not None) and (not np.array_equal(metadata, np.empty(len(solutions), dtype=object))):
            raise ValueError("This emitter does not (yet) store external metadata.")

        loop_data = zip(solutions, objective_values, behavior_values, self.last_ask_sample_sizes, self.last_solutions_origins, self.last_elite_origins)

        tmp_origin_performance = copy.deepcopy(self._origin_performance_template.copy())

        for i, (sol, obj, beh, meta, sol_origin, e_origin) in enumerate(loop_data):
        ...
        tmp_origin_performance[SolutionOrigins.crossover] = list(
            np.array(tmp_origin_performance[SolutionOrigins.co_mutation]) + np.array(tmp_origin_performance[SolutionOrigins.co_no_mutation])
        )
        self._origin_performance_over_time.append(tmp_origin_performance)
        self._rejections_over_time.append(
            (self._tmp_crossover_rejects, self._tmp_crossover_mutation_reject, self._tmp_no_crossover_mutation_reject, self._tmp_elite_origin_reject_counter)
        )
        ...
        for o_p in self._origin_performance_over_time[N:]:
            for k in self._origin_performance.keys():
                self._origin_performance[k] = list(np.array(self._origin_performance[k]) + np.array(o_p[k]))
        ...
        for rejcs in self._rejections_over_time[N:]:
            ...
            tmp_val_2 = {k: rejcs[-1][k] + self._rejections_counts[-1][k] for k in self._rejections_counts[-1].keys()}
        ...
        if (
            ...
            if self.dynamic_updates_consider_rejections:
                reject_ratio_crossover = self._origin_performance[SolutionOrigins.crossover][-1] / (
                    self._origin_performance[SolutionOrigins.crossover][-1] + self._rejections_counts[0] + self._rejections_counts[1]
                )
                reject_ratio_no_crossover = self._origin_performance[SolutionOrigins.no_crossover][-1] / (
                    self._origin_performance[SolutionOrigins.no_crossover][-1] + self._rejections_counts[2]
                )
                ...
                self._origin_performance[SolutionOrigins.no_crossover][0] *= reject_ratio_no_crossover

            # Update
            cr_prb = _avg_obj_update(self._origin_performance[SolutionOrigins.crossover], self._origin_performance[SolutionOrigins.no_crossover])
            self._crossover_probability = max(min(cr_prb, 0.9), 0.1)
    ...
    def _update_mutation_probability_after_crossover(self):
        if (
            ...
            # Consider reject rate in performance assessment
            if self.dynamic_updates_consider_rejections:
                reject_ratio_co_mutation = self._origin_performance[SolutionOrigins.co_mutation][-1] / (
                    self._origin_performance[SolutionOrigins.co_mutation][-1] + self._rejections_counts[1]
                )
                reject_ratio_co_no_mutation = self._origin_performance[SolutionOrigins.co_no_mutation][-1] / (
                    self._origin_performance[SolutionOrigins.co_no_mutation][-1] + self._rejections_counts[0]
                )
                ...
                self._origin_performance[SolutionOrigins.co_no_mutation][0] *= reject_ratio_co_no_mutation

            cr_prb = _avg_obj_update(self._origin_performance[SolutionOrigins.co_mutation], self._origin_performance[SolutionOrigins.co_no_mutation])

            self._mutation_probability_after_crossover = max(min(cr_prb, 0.9), 0.1)
        ...
        if (
            ...
            if self.dynamic_updates_consider_rejections:
                reject_ratio_stochastic = self._origin_performance[EliteOrigins.stochastic][-1] / (
                    self._origin_performance[EliteOrigins.stochastic][-1] + self._rejections_counts[-1][EliteOrigins.stochastic]
                )

                reject_ratio_deterministic = self._origin_performance[EliteOrigins.deterministic][-1] / (
                    self._origin_performance[EliteOrigins.deterministic][-1] + self._rejections_counts[-1][EliteOrigins.deterministic]
                )
                ...
                self._origin_performance[EliteOrigins.deterministic][0] *= reject_ratio_deterministic

            rng_prb = _avg_obj_update(self._origin_performance[EliteOrigins.stochastic], self._origin_performance[EliteOrigins.deterministic])
    ...
    def _update_base_model_sample_weights(self, solutions, objective_values):
        if self._weight_random_step_selection:
            ...
            if self.archive.empty:
                baseline_performance = np.max(objective_values)
            else:
                baseline_performance = self.archive.stats.obj_max

            # Compute the contribution to the performance for each base model; scale it by how much it contributed
            avg_perf_per_bm = np.divide((solutions * objective_values.reshape(-1, 1)).sum(axis=0), np.sum(solutions, axis=0))
            # Read as: The base model's contribution was X times worse than the baseline on average
            rel_dist_to_baseline = abs(avg_perf_per_bm - baseline_performance) / abs(baseline_performance)
            ...
            self._base_model_sample_weights = softmax(self._base_model_sample_weights - update_per_weight)
    ...
    def _get_mutated_children(self, use_crossover):
        ...
        if do_crossover:
            ...
            if (EliteOrigins.stochastic in parent_origins) and (EliteOrigins.deterministic in parent_origins):
            ...
            for c_elite_float_w, c_sample_size in children:
                ...
                do_mutation_after_crossover = self._rng.random() < (self.mutation_probability_after_crossover + self._emergency_mutation_rate)
        ...
        mutated_children = []
        for m_c, elite_origin in zip(tmp_mutated_children, tmp_elite_origins):
    ...
    def _get_random_elite(self):
        if self._weight_random_elite_selection:
            ...
            elites = [e for e in self.archive]
            if len(elites) == 1:
                ...
                sel_idx = self._rng.choice(len(elites), p=np.exp(performances) / np.exp(performances).sum())
    ...
    def _get_deterministic_elite(self, n=1):
        ...
        if n == 1:
            ...
            return [(elites[i_best].sol, elites[i_best].meta) for i_best in np.argpartition(performances, -n)[-n:]]
    ...
    def _edge_case_select_next_elite(self, n_elites=1):
        ...
        if n_elites > len(self._original_start_weight_vectors):
            ...
            while len(elites) < n_elites:
                ...
                sel_idx = self._rng.choice(
                    len(self._original_start_weight_vectors), min(n_elites - len(elites), len(self._original_start_weight_vectors)), replace=False
                )
                for ix in sel_idx:
                    # Mutate and add to list
                    *sampled_elite, reject_flag = self._mutate_elite(*self._original_start_weight_vectors[ix])
                    if not reject_flag:
        ...
        # -- Default case
            ...
            sel_idx = self._rng.choice(len(self._original_start_weight_vectors), n_elites, replace=False)
    ...
    def _tournament_selection(self, n_elites):
        ...
        if not elites:
            # -- Tournament
            all_elites = [e for e in self.archive]
            # Select without replacement
            elites = [all_elites[j] for j in self._rng.choice(len(all_elites), n_competitors, replace=False)]
            perf_elites = [e.obj for e in elites]
            elites = np.array([(elite_float_w, sample_size) for elite_float_w, _, _, _, sample_size in elites], dtype="object")
    ...
    def _crossover_average(self, p1_elite_dis_w, p2_elite_dis_w):
        ...
        children = [(proposed_float_weights, new_sample_size)]
        ...
        if self._weight_random_step_selection:
            selected_step = tuple(
                self._rng.choice(
                    ...
                    p=softmax([self._base_model_sample_weights[idx] for idx, _ in self._remaining_steps_for_hash[dwv_hash]]),
                )
            )
        else:
...
def _calculate_weights(discrete_weights) -> np.ndarray:
    ...
    weights = discrete_weights / np.abs(discrete_weights).sum()
